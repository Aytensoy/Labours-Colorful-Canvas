  ...
  modal.innerHTML = `
        <div class="premium-content">
            ...
            <div class="premium-features">
                <div class="feature">✨ All 21 Coloring Pages</div>
                <div class="feature">💾 Unlimited Saves</div>
                <div class="feature">🎨 Special Magic Brushes</div>
                <div class="feature">📱 Full Offline Mode</div>
                <div class="feature">🖼️ High Resolution Downloads</div>
                <div class="feature">🚫 No Daily Limits</div>
            </div>
            ...
            <p class="premium-note">Instant activation with license key</p>
        </div>
    `;
...
// Premium içerikleri kilitle/aç
function updateContentLocks() {
  const thumbnails = document.querySelectorAll('.page-thumbnail');

  thumbnails.forEach((thumb, index) => {
    // İlk 10 sayfa ücretsiz
...
function enableOfflineMode() {
  ...
  progress.innerHTML = `
        <div class="progress-content">
            <h3>Downloading All Content...</h3>
            <div class="progress-bar">
                <div class="progress-fill"></div>
            </div>
            <p>Please wait while we prepare offline mode</p>
        </div>
    `;
  ...
  setTimeout(() => {
    ...
    progress.querySelector('p').textContent = 'You can now use the app offline';
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  window.setTool = function (toolName) {
    ...
    switch (toolName) {
      ...
      case 'rainbow':
      case 'glow':
        ctx.lineWidth = brushSize; // Premium araçlar brush boyutunu kullanır
        break;
    }
...
// GERÇEKÇİ GÖKKUŞAĞI FIRÇASI - Eski drawRainbow fonksiyonunun yerine

// PASTEL GÖKKUŞAĞI - Eski drawRainbow yerine bunu kullanın
...
function drawRainbow(x, y) {
  ...
  const rainbowColors = [
    { color: '#FF919A', position: 0 },      // Pastel Kırmızı (dış)
    { color: '#FFCD99', position: 0.14 },   // Pastel Turuncu
    { color: '#FFF199', position: 0.28 },   // Pastel Sarı
    { color: '#9FE0AF', position: 0.42 },   // Pastel Yeşil
    { color: '#99CEFF', position: 0.57 },   // Pastel Mavi
    { color: '#B09FE3', position: 0.71 },   // Pastel Lacivert
    { color: '#FF9FE3', position: 0.85 }    // Pastel Mor (iç)
  ];
  ...
  for (let step = 0; step < steps; step++) {
    ...
    // Pastel renkler zaten yumuşak, hafif glow yeterli
    rainbowColors.forEach((colorInfo, index) => {
      ctx.save();

      // Çok hafif shadow
      ctx.shadowBlur = 4;
      ctx.shadowColor = colorInfo.color;

      ctx.strokeStyle = colorInfo.color;
      ctx.lineWidth = bandWidth + 2; // Bantlar arası boşluk olmasın
      ctx.lineCap = 'round';
      ctx.lineJoin = 'round';
      ctx.globalAlpha = 0.9; // Pastel renkler için yüksek opaklık
      ...
      ctx.beginPath();
      if (step === 0) {
        const prevBandX = lastX + perpX * offset;
        const prevBandY = lastY + perpY * offset;
...
// BONUS: Daha yumuşak pastel tonları isterseniz
const softPastelRainbow = [
  { color: '#FFC8CC', position: 0 },      // Çok açık pembe-kırmızı
  { color: '#FFE5CC', position: 0.14 },   // Çok açık şeftali
  { color: '#FFF9CC', position: 0.28 },   // Çok açık limon
  { color: '#D4FFDD', position: 0.42 },   // Çok açık mint
  { color: '#CCE7FF', position: 0.57 },   // Çok açık gök mavisi
  { color: '#DDD4FF', position: 0.71 },   // Çok açık lavanta
  { color: '#FFCCF5', position: 0.85 }    // Çok açık pembe-mor
];
